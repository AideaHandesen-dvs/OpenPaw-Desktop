...
def _setup_dbus_session() -> None:
    """
    DBUS_SESSION_BUS_ADDRESS が未設定の場合（SSH接続時など）、
    systemd ユーザーセッションバスのパスを自動補完する。
    GUIセッション（Konsole等）では既に設定済みのため何もしない。
    """
    if os.environ.get("DBUS_SESSION_BUS_ADDRESS"):
        return
    uid = os.getuid()
    candidate = Path(f"/run/user/{uid}/bus")
    if candidate.exists():
