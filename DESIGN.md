...
1. `python tests/test_prev_handoff.py -v` を実機で通す
2. `shell` ツールのパス制限設計（Section XX に追記予定）
   - `filesystem` は action 許可リストで守れたが `shell` は任意コマンドを実行できる
   - `command` 文字列からパスを抽出して `allowed_paths` でチェックする仕組みを検討
   - シェルコマンドの完全パースは困難なので「できる範囲でやる」設計にする
3. 実装・テスト
