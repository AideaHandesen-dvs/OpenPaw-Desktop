...
### [BUG-007] KRunner履歴クリアがD-Busで達成できない（意味理解の限界）

**発見日:** 2026-05-19
**症状:** `python main.py "KRunnerの履歴を消して"` で `toggleDisplay`×2が実行される。
「再起動すれば履歴が消える」と推論している可能性が高い（実際には消えない）。
**根本原因:**
1. KRunner履歴クリアのD-Busメソッドがそもそも存在しない
2. LLMが「存在するが意味的に間違ったメソッド」を選択するのをコードで防ぐ手段がない
3. 正解は `~/.config/krunnerrc` のshell編集だが、7bモデルはその判断に至れない

**正しい実行方法（手動）:**
```bash
sed -i '/^historyEnabled\|^history/d' ~/.config/krunnerrc
```
**対処方針:** 未定。タスク特化のルール追加（SYSTEM_PROMPTにkrunnerrc編集例）を
検討できるが、類似の「D-Busなしで解決すべきタスク」は無数にあるため汎用解にはならない。
